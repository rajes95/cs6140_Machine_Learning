...
Problem 1.2.2 - Dual Perceptron Classification - Perceptron Dataset
...
def tenFoldCrossValidation(originalData, target, learningRate, verbose=True):
    ...
    for n in range(10):
        ...
        weightsMatrix = pf.dualPerceptron(normTrainData, target, learningRate)
#         alphas = pf.dualPerceptronKernel(normTrainData, target, learningRate, pf.linearKernel)
...
#         testAccuracy, testConfMatrix = pf.calcAccuracyUsingAlphas(normTestData, normTrainData, target, alphas, pf.linearKernel)
        ...
        if verbose:
            print("\nFold #", n + 1)
            print("Weights for normalized data using Dual Perceptron Algorithm w/ linear kernel:")
            print(weightsMatrix)
