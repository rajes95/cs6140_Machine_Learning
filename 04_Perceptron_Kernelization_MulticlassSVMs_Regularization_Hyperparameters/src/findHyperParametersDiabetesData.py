...
Problem 3 - Determining Model Hyper-parameters - Diabetes Dataset RBF/Gaussian Kernel
Hyperparameters chosen by optimizing for Accuracy
...
def nestedCrossValidation(data, cVals, gammaVals, kFolds=10, mFolds=5):
    ...
    resultsTable = pd.DataFrame(columns=["OuterFold", "cVal", "gammaVal", "TrainAccuracy", \
...
#         print(bestC, bestGamma, bestAcc)
        ...
        # gaussian kernel
        classifier = SVC(kernel='rbf', C=bestC, gamma=bestGamma)
        ...
        resultsTable.loc[len(resultsTable)] = [k + 1, bestC, bestGamma, accTrain, precTrain, recTrain, acc, prec, rec]
...
#     pd.set_option('display.max_rows', None)
    ...
    colNames = range(9)
    try:
        diabData = pd.read_csv("datasets/diabetes.csv", names=colNames)
    except:
        print("File not found")

    print(diabData)

    cVals = [2 ** -5, 2 ** -3, 2 ** 1, 2 ** 2, 2 ** 3, 2 ** 5, 2 ** 8, 2 ** 10]
    gammaVals = [2 ** -15, 2 ** -12, 2 ** -9, 2 ** -3, 1, 2 ** 1, 2 ** 3, 2 ** 5]

    results = nestedCrossValidation(diabData, cVals, gammaVals)
