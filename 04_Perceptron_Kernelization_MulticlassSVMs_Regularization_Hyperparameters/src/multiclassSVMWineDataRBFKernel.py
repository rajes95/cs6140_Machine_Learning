...
Problem 4 - SVMs vs Multiclass Problems - Wine Dataset Gaussian/RBF Kernel
...
def nestedCrossValidation(data, cVals, gammaVals, kFolds=10, mFolds=5):
    ...
    resultsTable1 = pd.DataFrame(columns=["OuterFold", "cVal", "gammaVal", "TrainAccuracy", \
            "TrainPrecision", "TrainRecall", "TestAccuracy", "TestPrecision", "TestRecall"])
    resultsTable2 = pd.DataFrame(columns=["OuterFold", "cVal", "gammaVal", "TrainAccuracy", \
            "TrainPrecision", "TrainRecall", "TestAccuracy", "TestPrecision", "TestRecall"])
    resultsTable3 = pd.DataFrame(columns=["OuterFold", "cVal", "gammaVal", "TrainAccuracy", \
            "TrainPrecision", "TrainRecall", "TestAccuracy", "TestPrecision", "TestRecall"])
    ...
    for k in range(kFolds):
        ...
        for m in range(mFolds):
            ...
            for g in gammaVals:
                for c in cVals:
                    ...
                    # gaussian kernel
                    classifier1 = SVC(kernel='rbf', C=c, gamma=g, probability=True)
                    classifier2 = SVC(kernel='rbf', C=c, gamma=g, probability=True)
                    classifier3 = SVC(kernel='rbf', C=c, gamma=g, probability=True)
            ...
            print("Kfold:", k + 1, "Mfold:", m + 1, "Class1BestC:", bestC1, "Class1BestGamma:", \
                   bestGamma1, "Class2BestC:", bestC2, "Class2BestGamma:", bestGamma2, \
                    "Class3BestC:", bestC3, "Class3BestGamma:", bestGamma3)
        ...
        # gaussian kernel
        classifier1 = SVC(kernel='rbf', C=bestC1, gamma=bestGamma1, probability=True)
        classifier2 = SVC(kernel='rbf', C=bestC2, gamma=bestGamma2, probability=True)
        classifier3 = SVC(kernel='rbf', C=bestC3, gamma=bestGamma3, probability=True)
        ...
        resultsTable1.loc[len(resultsTable1)] = [k + 1, bestC1, bestGamma1, acc1Tr, prec1Tr, rec1Tr, acc1, prec1, rec1]
        resultsTable2.loc[len(resultsTable2)] = [k + 1, bestC2, bestGamma2, acc2Tr, prec2Tr, rec2Tr, acc2, prec2, rec2]
        resultsTable3.loc[len(resultsTable3)] = [k + 1, bestC3, bestGamma3, acc3Tr, prec3Tr, rec3Tr, acc3, prec3, rec3]
    ...
    gammaList = resultsTable1["gammaVal"].tolist()
    ...
    resultsTable1.loc[len(resultsTable1)] = ["Avg", st.mean(cList), st.mean(gammaList), \
        st.mean(accListTr), st.mean(precListTr), st.mean(recListTr), st.mean(accList), st.mean(precList), st.mean(recList)]
    resultsTable1.loc[len(resultsTable1)] = ["StdDev", st.stdev(cList), st.stdev(gammaList), \
    ...
    gammaList = resultsTable2["gammaVal"].tolist()
    ...
    resultsTable2.loc[len(resultsTable2)] = ["Avg", st.mean(cList), st.mean(gammaList), \
        st.mean(accListTr), st.mean(precListTr), st.mean(recListTr), st.mean(accList), st.mean(precList), st.mean(recList)]
    resultsTable2.loc[len(resultsTable2)] = ["StdDev", st.stdev(cList), st.stdev(gammaList), \
        st.stdev(accListTr), st.stdev(precListTr), st.stdev(recListTr), st.stdev(accList), st.stdev(precList), st.stdev(recList)]

    cList = resultsTable3["cVal"].tolist()
    gammaList = resultsTable3["gammaVal"].tolist()
    ...
    resultsTable3.loc[len(resultsTable3)] = ["Avg", st.mean(cList), st.mean(gammaList), \
        st.mean(accListTr), st.mean(precListTr), st.mean(recListTr), st.mean(accList), st.mean(precList), st.mean(recList)]
    resultsTable3.loc[len(resultsTable3)] = ["StdDev", st.stdev(cList), st.stdev(gammaList), \
        st.stdev(accListTr), st.stdev(precListTr), st.stdev(recListTr), st.stdev(accList), st.stdev(precList), st.stdev(recList)]
...
#     pd.set_option('display.max_rows', None)
 ...
 = nestedCrossValidation(wineData, cVals, gammaVals)
